...
(function() {
  ...
  Batman.DOM.EditableBinding = (function(_super) {
    var onlyAll, onlyData, onlyNode;

    __extends(EditableBinding, _super);

    function EditableBinding() {
      _ref = EditableBinding.__super__.constructor.apply(this, arguments);
      return _ref;
    }

    EditableBinding.prototype.nodeChange = function(node, context) {
      if (this.isTwoWay()) {
        return this.set('filteredValue', this.node.textContent);
      }
    };

    EditableBinding.prototype.dataChange = function(value, node) {
      return jQuery(this.node).text(value);
    };
    ...
    EditableBinding.prototype.bind = function() {
      var _ref1, _ref2;
      if (this.node && ((_ref1 = this.onlyObserve) === onlyAll || _ref1 === onlyNode)) {
        this._setupEvents(this.node, this._fireNodeChange.bind(this));
        if (this.onlyObserve === onlyNode) {
          this._fireNodeChange();
        }
      }
    ...
    EditableBinding.prototype._setupEvents = function(node, callback, view) {
      var eventName, _i, _len, _ref1, _results;
      Batman.DOM.addEventListener(node, 'focus', function() {
        var args, self;
        args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        self = $(this);
        return self.data('before', self.html());
      });
      _ref1 = ['change', 'blur', 'keyup', 'keypress', 'keydown', 'paste'];
      _results = [];
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        eventName = _ref1[_i];
  ...
  Batman.DOM.readers.editable = function(definition) {
    return new Batman.DOM.EditableBinding(definition);
  };
